...
- **Folding** ranges come from `foldService.of(...)` ([src/editor/extension.ts](src/editor/extension.ts), shared by both surfaces). The Markdown editor relies on Obsidian's existing fold gutter/state (don't add our own there — it would duplicate and conflict). The standalone `YamlFileView` is a raw CodeMirror instance with no fold machinery, so it adds its own `codeFolding()` + `foldGutter()` + `foldKeymap` ([src/ui/yamlview.ts](src/ui/yamlview.ts)).
