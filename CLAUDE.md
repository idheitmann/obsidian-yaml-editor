...
3. **`SchemaTracker`** ([src/yaml/schema.ts](src/yaml/schema.ts)) — the only vault-scoped piece. Scans every markdown file's YAML, folds observed `(path → kinds, examples, count)` into a probabilistic schema, and updates incrementally on vault modify/create/delete/rename events. Both layers query it via `keysAt(path)`.

### The core data flow

Every doc edit recomputes editor state in one place: `yamlStateField` ([src/editor/state.ts](src/editor/state.ts)). On `docChanged` it calls `findYamlRegions` → `parseRegion` for each region and stores regions + per-region parse errors. Downstream consumers (`completions`, `affordances`, `decorations`, the `linter`) all read this field rather than reparsing — **except** they currently often re-call `findYamlRegions(doc)` themselves; prefer reading `yamlStateField` when adding code.

### The two YAML-understanding primitives

- **`findYamlRegions(doc)`** ([src/yaml/regions.ts](src/yaml/regions.ts)) — pure string scan that locates frontmatter + fenced YAML and returns `{from, to, text, kind}`. Cheap; called on every change. Everything operates on a region's local `text` and translates back via `region.from`.
